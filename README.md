...
GitOps manifests for deploying demo Fedora virtual machines on OpenShift Virtualization with Argo CD / OpenShift GitOps. This branch targets a workshop cluster.
...
└── manifests/
    ├── kustomization.yaml           # Points at overlays/workshop
    ├── base/
    │   ├── kustomization.yaml
...
| Cloud-init overlay | `manifests/cloud-init/overlays/workshop/` | Per-VM `userdata` and `networkdata` secrets |
| Workshop overlay | `manifests/overlays/workshop/` | Composes base, VMs, and cloud-init; patches Multus network |
...
- `spec.source.path` — `manifests/overlays/workshop`
- `spec.source.targetRevision` — branch to track (`main`)
...
Argo CD syncs `manifests/overlays/workshop` into `berto-virt-gitops-demo` with automated prune and self-heal.
...
- **VM count / names** — add `manifests/virtualmachines/overlays/vm-XX/`, list it in `manifests/overlays/workshop/kustomization.yaml`, and add matching `userdata/vm-XX` and `networkdata/vm-XX` files plus a `secretGenerator` entry in `manifests/cloud-init/overlays/workshop/kustomization.yaml`
- **User / password / SSH key** — edit `manifests/cloud-init/overlays/workshop/userdata/vm-*`
- **IPs / DNS / gateway** — edit `manifests/cloud-init/overlays/workshop/networkdata/vm-*`
- **Multus network** — edit `manifests/overlays/workshop/patches/multus-network.yaml`
